  ...
  return (
    <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm p-6 flex flex-col gap-4">
      <p className="text-xs font-semibold text-violet-500 uppercase tracking-wide">
        {tituloPorDificultad(dificultad)}
      </p>

      <div className="text-center my-2">
        <p className="text-sm text-gray-600 mb-1">
          Resolv&eacute; la siguiente operación:
        </p>
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-3 mt-2">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            value={respuesta}
            onChange={(e) => onChangeRespuesta(e.target.value)}
            disabled={resuelto}
          />
          {error && (
